export const dummyLocalCategories = [
  {
    id: 1,
    name: "Manado - Minahasa - Likupang",
    slug: "manado-minahasa-likupang",
    description: "The complete North Sulawesi experience.",
    type: "nature",
  },
  {
    id: 2,
    name: "Tangkoko Adventure Tour",
    slug: "tangkoko-adventure",
    description: "Wildlife and nature expedition.",
    type: "nature",
  },
  {
    id: 3,
    name: "Makassar - Toraja",
    slug: "makassar-toraja",
    description: "Cultural journey in South Sulawesi.",
    type: "culture",
  },
];

export const dummyLocalTours = [
  {
    id: 1,
    category_id: 1,
    title: "Manado City Tour",
    slug: "manado-city-tour",
    summary: "Tur satu hari penuh menjelajahi landmark kota Manado.",
    description:
      "Peserta tour akan dijemput oleh pemandu wisata dan langsung memulai tour di kota Manado dengan menuju ke Kawasan Ring Road Citra Land dimana peserta tour dapat melihat dari dekat Monumen Yesus Memberkati; patung Yesus yang dibangun dengan posisi melayang menghadap kota Manado. Kemudian dilanjutkan dengan City Tour : menuju pusat kota Manado, Kawasan Pecinan, Klenteng Ban Hin Kiong (Klenteng tertua di Manado yang dibangun pada abad ke-18, berumur ±300 tahun), melewati Gereja Sentrum (Gereja Protestan pertama di Sulawesi Utara peninggalan zaman Belanda) dan Monumen Perang Dunia ke II (bukti kehancuran kota Manado tahun 1942 oleh serdadu-serdadu Jepang). Selanjutnya kita menuju ke lokal restoran untuk makan siang makanan khas Manado (halal), setelah itu kita menuju ke MAKATETE HILLS yang terletak di atas puncak ketinggian 200M diatas permukaan laut, kita bisa menyaksikan pemandangan kota Manado, hamparan lautan luas, Pulau Manado Tua, Pulau Bunaken, Pulau Siladen serta pemandangan gunung Klabat, gunung tertinggi di Sulawesi Utara. Pelayanan makan malam disediakan di lokal restoran di kota Manado. Setelah makan malam, diantar ke hotel untuk proses check in.",
    base_price: 750000,
    duration_days: 1,
    duration_nights: 0,
    location: "Manado, Sulawesi Utara",
    category: {
      slug: "manado-minahasa-likupang",
      name: "Manado - Minahasa - Likupang",
    },
    price_details: [
      { pax_range: "2-3 Pax", price_per_pax: 950000 },
      { pax_range: "4-6 Pax", price_per_pax: 750000 },
      { pax_range: "7-10 Pax", price_per_pax: 650000 },
    ],
    galleries: [
      {
        image_path:
          "https://images.unsplash.com/photo-1549294413-26f195200c16?auto=format&fit=crop&w=800&q=80",
      },
      {
        image_path:
          "https://images.unsplash.com/photo-1518391846015-55a9cc003b25?auto=format&fit=crop&w=800&q=80",
      },
      {
        image_path:
          "https://images.unsplash.com/photo-1544551763-46a013bb70d5?auto=format&fit=crop&w=800&q=80",
      },
      {
        image_path:
          "https://images.unsplash.com/photo-1502791451862-7bd8c1df43a7?auto=format&fit=crop&w=800&q=80",
      },
      {
        image_path:
          "https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?auto=format&fit=crop&w=800&q=80",
      },
      {
        image_path:
          "https://images.unsplash.com/photo-1540573133985-87b6da6d54a9?auto=format&fit=crop&w=800&q=80",
      },
    ],
    itineraries: [
      {
        id: 1,
        day_number: 1,
        title: "Penjemputan & City Tour",
        description:
          "Dijemput di Hotel/Bandara, mengunjungi Monumen Yesus Memberkati, Pecinan, Klenteng Ban Hin Kiong, dan Makatete Hills.",
        hotel_info: "-",
        meals_info: "Lunch & Dinner",
      },
    ],
  },
  {
    id: 3,
    category_id: 1,
    title: "Minahasa Highland Tour",
    slug: "minahasa-highland-tour",
    summary: "Menikmati udara sejuk pegunungan and budaya Minahasa.",
    description:
      "Perjalanan ke dataran tinggi Minahasa yang sejuk. Mengunjungi Tomohon, Danau Linow, and Pagoda Ekayana.",
    base_price: 950000,
    duration_days: 1,
    duration_nights: 0,
    location: "Tomohon, Sulawesi Utara",
    category: {
      slug: "manado-minahasa-likupang",
      name: "Manado - Minahasa - Likupang",
    },
    galleries: [
      {
        image_path:
          "https://images.unsplash.com/photo-1518391846015-55a9cc003b25?auto=format&fit=crop&w=800&q=80",
      },
    ],
    itineraries: [
      {
        id: 3,
        day_number: 1,
        title: "Pasar Beriman",
        description: "Mengunjungi pasar ekstrem Tomohon.",
        hotel_info: "-",
        meals_info: "Sarapan",
      },
    ],
  },
  {
    id: 2,
    category_id: 1,
    title: "Bunaken Marine Park Tour",
    slug: "bunaken-marine-park",
    summary: "Jelajahi keindahan bawah laut Taman Nasional Bunaken.",
    description:
      "Bunaken adalah surga bagi para pecinta snorkeling. Nikmati keindahan bawah laut and pulau yang mempesona.",
    base_price: 1200000,
    duration_days: 1,
    duration_nights: 0,
    location: "Pulau Bunaken",
    category: {
      slug: "manado-minahasa-likupang",
      name: "Manado - Minahasa - Likupang",
    },
    galleries: [
      {
        image_path:
          "https://images.unsplash.com/photo-1544551763-46a013bb70d5?auto=format&fit=crop&w=800&q=80",
      },
    ],
    itineraries: [
      {
        id: 2,
        day_number: 1,
        title: "Berangkat",
        description: "Menuju Dermaga Kalimas and menyeberang ke Bunaken.",
        hotel_info: "-",
        meals_info: "Lunch",
      },
    ],
  },
  {
    id: 31,
    category_id: 1,
    title: "Bunaken Discovery Fun Scuba Dive",
    slug: "bunaken-discovery-dive",
    summary: "Pengalaman menyelam pertama bagi pemula di Bunaken.",
    description:
      "Rasakan sensasi bernapas di bawah laut dengan bimbingan instruktur profesional.",
    base_price: 1500000,
    duration_days: 1,
    duration_nights: 0,
    location: "Taman Nasional Bunaken",
    category: {
      slug: "manado-minahasa-likupang",
      name: "Manado - Minahasa - Likupang",
    },
    galleries: [
      {
        image_path:
          "https://images.unsplash.com/photo-1544551763-46a013bb70d5?auto=format&fit=crop&w=800&q=80",
      },
    ],
    itineraries: [
      {
        id: 31,
        day_number: 1,
        title: "Briefing",
        description: "Latihan dasar menyelam di perairan dangkal.",
        hotel_info: "-",
        meals_info: "-",
      },
    ],
  },
  {
    id: 32,
    category_id: 1,
    title: "Likupang Lihaga Island Tour",
    slug: "likupang-lihaga-tour",
    summary: "Pulau pasir putih tak berpenghuni yang eksotis di Likupang.",
    description:
      "Nikmati ketenangan di Pulau Lihaga dengan pasir putih yang sangat halus and air jernih.",
    base_price: 1350000,
    duration_days: 1,
    duration_nights: 0,
    location: "Likupang",
    category: {
      slug: "manado-minahasa-likupang",
      name: "Manado - Minahasa - Likupang",
    },
    galleries: [
      {
        image_path:
          "https://images.unsplash.com/photo-1502791451862-7bd8c1df43a7?auto=format&fit=crop&w=800&q=80",
      },
    ],
    itineraries: [
      {
        id: 32,
        day_number: 1,
        title: "Pulau Lihaga",
        description: "Menikmati keindahan pantai and snorkeling.",
        hotel_info: "-",
        meals_info: "Lunch Box",
      },
    ],
  },
  {
    id: 33,
    category_id: 1,
    title: "Whitewater Rafting Tour",
    slug: "whitewater-rafting-tour",
    summary: "Petualangan arung jeram yang memacu adrenalin di Sungai Nimanga.",
    description:
      "Rasakan serunya mengarungi jeram sungai dengan pemandangan hutan tropis yang asri.",
    base_price: 1100000,
    duration_days: 1,
    duration_nights: 0,
    location: "Minahasa",
    category: {
      slug: "manado-minahasa-likupang",
      name: "Manado - Minahasa - Likupang",
    },
    galleries: [
      {
        image_path:
          "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=800&q=80",
      },
    ],
    itineraries: [
      {
        id: 33,
        day_number: 1,
        title: "Rafting",
        description: "Persiapan and mulai pengarungan sungai.",
        hotel_info: "-",
        meals_info: "Lunch",
      },
    ],
  },
  {
    id: 25,
    category_id: 1,
    title: "Mount Mahawu Volcano Trek",
    slug: "mount-mahawu-trek",
    summary: "Melihat kawah belerang and pemandangan kota Tomohon.",
    description:
      "Trekking ringan menuju puncak Gunung Mahawu untuk melihat kawah mati and kota bunga.",
    base_price: 800000,
    duration_days: 1,
    duration_nights: 0,
    location: "Tomohon",
    category: {
      slug: "manado-minahasa-likupang",
      name: "Manado - Minahasa - Likupang",
    },
    galleries: [
      {
        image_path:
          "https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?auto=format&fit=crop&w=800&q=80",
      },
    ],
    itineraries: [
      {
        id: 25,
        day_number: 1,
        title: "Summit",
        description: "Trekking santai menuju kawah.",
        hotel_info: "-",
        meals_info: "-",
      },
    ],
  },
  {
    id: 34,
    category_id: 1,
    title: "Paragliding Adventure Manado",
    slug: "paragliding-tumpa",
    summary: "Terbang tandem and nikmati pemandangan Teluk Manado.",
    description:
      "Paragliding di Gunung Tumpa adalah pengalaman tak terlupakan bagi para penggemar olahraga ekstrem.",
    base_price: null,
    duration_days: 0,
    duration_nights: 0,
    duration_hours: 3,
    location: "Manado",
    category: {
      slug: "manado-minahasa-likupang",
      name: "Manado - Minahasa - Likupang",
    },
    galleries: [
      {
        image_path:
          "https://images.unsplash.com/photo-1518391846015-55a9cc003b25?auto=format&fit=crop&w=800&q=80",
      },
    ],
    itineraries: [
      {
        id: 34,
        day_number: 1,
        title: "Duration",
        description: "3 hours adventure.",
        hotel_info: "-",
        meals_info: "No Meal",
      },
    ],
  },
  {
    id: 35,
    category_id: 1,
    title: "5D4N / 4D3N / 3D2N TOUR PACKAGE",
    slug: "tour-package-multi-day",
    summary: "Paket lengkap menginap and wisata di Sulawesi Utara.",
    description:
      "Pilihan paket wisata lengkap dengan akomodasi hotel and transportasi selama kunjungan. Menjelajahi Manado, Bunaken, dan Dataran Tinggi Minahasa dalam satu paket perjalanan eksklusif.",
    base_price: 3500000,
    duration_days: 5,
    duration_nights: 4,
    location: "North Sulawesi",
    category: {
      slug: "manado-minahasa-likupang",
      name: "Manado - Minahasa - Likupang",
    },
    galleries: [
      {
        image_path:
          "https://images.unsplash.com/photo-1549294413-26f195200c16?auto=format&fit=crop&w=800&q=80",
      },
    ],
    itineraries: [
      {
        id: 10,
        day_number: 1,
        title: "Arrival & City Tour",
        description: "Arrival at Sam Ratulangi Airport and Manado City Tour.",
        hotel_info: "4-star Hotel",
        meals_info: "Dinner",
      },
      {
        id: 11,
        day_number: 2,
        title: "Bunaken Marine Park",
        description: "Full day at Bunaken for snorkeling and sightseeing.",
        hotel_info: "4-star Hotel",
        meals_info: "Breakfast, Lunch",
      },
      {
        id: 12,
        day_number: 3,
        title: "Minahasa Highland",
        description: "Visiting Tomohon, Lake Linow, and Pagoda Ekayana.",
        hotel_info: "4-star Hotel",
        meals_info: "Breakfast, Lunch",
      },
      {
        id: 13,
        day_number: 4,
        title: "Tangkoko Nature Reserve",
        description: "Wildlife spotting at Tangkoko (Tarsius and Macaca).",
        hotel_info: "4-star Hotel",
        meals_info: "Breakfast, Dinner",
      },
      {
        id: 14,
        day_number: 5,
        title: "Shopping & Departure",
        description: "Souvenir shopping and transfer to airport.",
        hotel_info: "-",
        meals_info: "Breakfast",
      },
    ],
  },
  {
    id: 12,
    category_id: 2,
    title: "Tangkoko Nature Reserve Tour",
    slug: "tangkoko-nature-reserve",
    summary: "Bertemu Tarsius and Monyet Hitam di habitat aslinya.",
    description:
      "Petualangan menembus hutan Tangkoko untuk melihat primata terkecil di dunia. Tangkoko adalah rumah bagi Tarsius Spectrum, monyet terkecil di dunia yang aktif di malam hari, serta Monyet Hitam Sulawesi (Macaca Nigra). Tour ini dimulai pada sore hari untuk memaksimalkan peluang melihat satwa-satwa endemik ini saat mereka mulai beraktivitas.",
    base_price: 850000,
    duration_days: 1,
    duration_nights: 0,
    location: "Bitung, North Sulawesi",
    category: { slug: "tangkoko-adventure", name: "Tangkoko Adventure Tour" },
    galleries: [
      {
        image_path:
          "https://images.unsplash.com/photo-1540573133985-87b6da6d54a9?auto=format&fit=crop&w=800&q=80",
      },
      {
        image_path:
          "https://images.unsplash.com/photo-1516690561799-46d8f74f9abf?auto=format&fit=crop&w=800&q=80",
      },
    ],
    itineraries: [
      {
        id: 12,
        day_number: 1,
        title: "Evening Trek",
        description: "Trekking sore hari mencari Tarsius and Macaca Nigra.",
        hotel_info: "-",
        meals_info: "Dinner",
      },
    ],
  },
  {
    id: 13,
    category_id: 2,
    title: "Tangkoko Bird Watching Tour",
    slug: "tangkoko-bird-watching",
    summary: "Surga bagi pengamat burung dengan puluhan spesies endemik.",
    description:
      "Jelajahi keanekaragaman avifauna di Tangkoko. Temukan Burung Rangkong (Knobbed Hornbill), Raja Udang (Kingfisher), dan spesies endemik lainnya dalam perjalanan satu hari yang mendalam.",
    base_price: 950000,
    duration_days: 1,
    duration_nights: 0,
    location: "Tangkoko Nature Reserve",
    category: { slug: "tangkoko-adventure", name: "Tangkoko Adventure Tour" },
    galleries: [
      {
        image_path:
          "https://images.unsplash.com/photo-1444464666168-49d633b867ad?auto=format&fit=crop&w=800&q=80",
      },
    ],
    itineraries: [
      {
        id: 13,
        day_number: 1,
        title: "Morning Birding",
        description: "Pengamatan burung di pagi hari saat aktivitas puncak.",
        hotel_info: "-",
        meals_info: "Lunch",
      },
    ],
  },
  {
    id: 14,
    category_id: 2,
    title: "2D1N Tangkoko Overnight Expedition",
    slug: "tangkoko-overnight",
    summary: "Pengalaman mendalam menginap di sekitar kawasan hutan.",
    description:
      "Nikmati petualangan dua hari di Tangkoko. Termasuk trekking malam untuk melihat Tarsius dan trekking pagi untuk melihat Macaca Nigra serta burung-burung eksotis. Menginap di homestay lokal yang nyaman untuk merasakan suasana hutan yang tenang.",
    base_price: 1850000,
    duration_days: 2,
    duration_nights: 1,
    location: "Batuputih, Bitung",
    category: { slug: "tangkoko-adventure", name: "Tangkoko Adventure Tour" },
    galleries: [
      {
        image_path:
          "https://images.unsplash.com/photo-1501854140801-50d01698950b?auto=format&fit=crop&w=800&q=80",
      },
    ],
    itineraries: [
      {
        id: 15,
        day_number: 1,
        title: "Arrival & Night Trek",
        description: "Tiba di Batuputih dan persiapan trekking malam.",
        hotel_info: "Local Homestay",
        meals_info: "Dinner",
      },
      {
        id: 16,
        day_number: 2,
        title: "Morning Trek & Departure",
        description: "Mencari Macaca Nigra dan Hornbill sebelum kembali.",
        hotel_info: "-",
        meals_info: "Breakfast, Lunch",
      },
    ],
  },
  {
    id: 15,
    category_id: 2,
    title: "3D2N Wildlife Photography Masterclass",
    slug: "tangkoko-photography-package",
    summary: "Paket khusus bagi fotografer alam liar profesional.",
    description:
      "Paket intensif 3 hari yang dirancang khusus untuk fotografer. Dipandu oleh ranger berpengalaman yang memahami perilaku satwa, memberikan Anda kesempatan terbaik untuk menangkap momen langka Tarsius, Macaca, dan Burung Rangkong.",
    base_price: 3200000,
    duration_days: 3,
    duration_nights: 2,
    location: "Tangkoko Nature Reserve",
    category: { slug: "tangkoko-adventure", name: "Tangkoko Adventure Tour" },
    galleries: [
      {
        image_path:
          "https://images.unsplash.com/photo-1542038784456-1ea8e935640e?auto=format&fit=crop&w=800&q=80",
      },
    ],
    itineraries: [
      {
        id: 17,
        day_number: 1,
        title: "Initial Scouting",
        description: "Mencari lokasi sarang Tarsius dan Macaca.",
        hotel_info: "Eco Lodge",
        meals_info: "Dinner",
      },
      {
        id: 18,
        day_number: 2,
        title: "Full Day Shooting",
        description: "Sesi foto pagi dan sore di berbagai titik strategis.",
        hotel_info: "Eco Lodge",
        meals_info: "B, L, D",
      },
      {
        id: 19,
        day_number: 3,
        title: "Birding Session & Wrap-up",
        description: "Fokus pada avifauna sebelum sesi berakhir.",
        hotel_info: "-",
        meals_info: "Breakfast",
      },
    ],
  },
  {
    id: 28,
    category_id: 3,
    title: "Toraja Cultural Heritage",
    slug: "toraja-cultural-heritage",
    summary: "Menyelami tradisi pemakaman and budaya unik suku Toraja.",
    description:
      "Kunjungi Lemo, Londa, and Kete Kesu untuk melihat makam di tebing batu and rumah adat Tongkonan.",
    base_price: 3800000,
    duration_days: 4,
    duration_nights: 3,
    location: "Tana Toraja, Sulawesi Selatan",
    category: { slug: "makassar-toraja", name: "Makassar - Toraja" },
    galleries: [
      {
        image_path:
          "https://images.unsplash.com/photo-1516690561799-46d8f74f9abf?auto=format&fit=crop&w=800&q=80",
      },
    ],
    itineraries: [
      {
        id: 28,
        day_number: 1,
        title: "Stone Grave Visit",
        description: "Melihat patung Tau-Tau di tebing batu Lemo.",
        hotel_info: "Heritage Hotel",
        meals_info: "B, L, D",
      },
    ],
  },
];
